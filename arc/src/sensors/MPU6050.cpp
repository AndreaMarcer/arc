...
/**
 * @brief When this bit is equal to 0, the logic level for the INT pin is active
 * high. When this bit is equal to 1, the logic level for the INT pin is active
 * low.
 */
...
/**
 * @brief When this bit is equal to 0, interrupt status bits are cleared only by
 * reading INT_STATUS (Register 58). When this bit is equal to 1, interrupt
 * status bits are cleared on any read operation.
 */
...
/**
 * @brief When set to 1, this bit enables the Data Ready interrupt, which occurs
...
MPU6050::MPU6050(i2c_inst_t *i2c_inst, uint8_t address)
    : m_i2c_inst{i2c_inst}, m_addr{address} {
